...
Operation::~Operation() { dissociate_item(_label); }
...
void Operation::from_container(const gpds::container &container) {
  // Root
  QSchematic::Items::Node::from_container(
      *container.get_value<gpds::container *>("node").value());
  _label->from_container(
      *container.get_value<gpds::container *>("label").value());
}
...
std::shared_ptr<QSchematic::Items::Item> Operation::deepCopy() const {
  auto clone =
      std::make_shared<Operation>(::ItemType::OperationType, parentItem());
...
void Operation::copyAttributes(Operation &dest) const {
  ...
  // Label
  dest._label =
      std::dynamic_pointer_cast<QSchematic::Items::Label>(_label->deepCopy());
...
void Operation::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget *widget) {
  Q_UNUSED(option)
  Q_UNUSED(widget)
  ...
  QMenu menu;
  {
    ...
    connect(text, &QAction::triggered, [this] {
      ...
      const QString &newText = QInputDialog::getText(
          nullptr, "Rename Connector", "New connector text", QLineEdit::Normal,
          label()->text(), &ok);
      if (!ok)
        return;

      scene()->undoStack()->push(
          new QSchematic::Commands::LabelRename(label().get(), newText));
    });
    ...
    connect(isMovable, &QAction::toggled,
            [this](bool enabled) { setMovable(enabled); });
    ...
    connect(labelVisibility, &QAction::toggled, [this](bool enabled) {
      ...
      scene()->undoStack()->push(
          new QSchematic::Commands::ItemVisibility(label(), enabled));
    });

    // Align label
    QAction *alignLabel = new QAction;
    alignLabel->setText("Align label");
    connect(alignLabel, &QAction::triggered, [this] { this->alignLabel(); });

    // Align connector labels
    QAction *alignConnectorLabels = new QAction;
    alignConnectorLabels->setText("Align connector labels");
    connect(alignConnectorLabels, &QAction::triggered,
            [this] { this->alignConnectorLabels(); });
    ...
    connect(showAllConnectors, &QAction::triggered, [this] {
      ...
      for (const std::shared_ptr<Items::Connector> &conn : connectors())
        scene()->undoStack()->push(
            new QSchematic::Commands::ItemVisibility(conn, true));
    });
    ...
    connect(newConnector, &QAction::triggered, [this, event] {
      ...
      auto connector = std::make_shared<OperationConnector>(
          event->pos().toPoint(), QStringLiteral("Unnamed"), this);

      scene()->undoStack()->push(
          new ::Commands::NodeAddConnector(this, connector));
    });
    ...
    connect(duplicate, &QAction::triggered, [this] {
      ...
      auto clone = deepCopy();
      clone->setPos(pos() +
                    QPointF(5 * _settings.gridSize, 5 * _settings.gridSize));
    ...
    connect(deleteFromModel, &QAction::triggered, [this] {
      ...
      // Issue command
      scene()->undoStack()->push(
          new QSchematic::Commands::ItemRemove(scene(), itemPointer));
    });

...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    auto func =
        std::bind(&CustomItemFactory::from_container, std::placeholders::_1);
    ...
    connect(_view, &QSchematic::View::zoomChanged, _itemLibraryWidget,
            &Library::Widget::setPixmapScale);
...
bool MainWindow::save() {
    ...
    const std::filesystem::path path =
        QFileDialog::getSaveFileName(this, "Save to file", QDir::homePath(),
                                     FILE_FILTERS)
            .toStdString();
    if (path.empty())
        return false;
    ...
    const auto &[success, message] =
        gpds::to_file<gpds::archiver_xml>(path, *_scene);
    if (!success) {
...
bool MainWindow::load(const QString &filepath) {
    // Note: We don't use gpds::from_file() in here because we're potentially
    // loading demo files from Qt's resource
    //       system. In such a case, those files can't be accessed via
    //       std::filesystem::path.
    ...
    // Archiver
    const auto &[success, message] = gpds::from_stream<gpds::archiver_xml>(
        stream, *_scene, QSchematic::Scene::gpds_name);
    if (!success) {
        qDebug() << "MainWindow::load(): Could not load scene: "
                 << QString::fromStdString(message);
        return false;
    }
...
void MainWindow::createActions() {
    ...
    connect(_actionOpen, &QAction::triggered, [this] { load(); });
    ...
    connect(_actionSave, &QAction::triggered, [this] { save(); });
    ...
    connect(_actionPrint, &QAction::triggered, [this] { print(); });

    // Undo
    _actionUndo =
        _scene->undoStack()->createUndoAction(this, QStringLiteral("Undo"));
    ...
    // Redo
    _actionRedo =
        _scene->undoStack()->createRedoAction(this, QStringLiteral("Redo"));
    ...
    _actionModeNormal->setToolTip(
        "Change to the normal mode (allows to move components).");
    _actionModeNormal->setCheckable(true);
    _actionModeNormal->setChecked(true);
    connect(_actionModeNormal, &QAction::triggered,
            [this] { _scene->setMode(QSchematic::Scene::NormalMode); });

    // Mode: Wire
    _actionModeWire = new QAction("Wire Mode", this);
    _actionModeWire->setIcon(QIcon(":/mode_wire.svg"));
    _actionModeWire->setToolTip(
        "Change to the wire mode (allows to draw wires).");
    _actionModeWire->setCheckable(true);
    connect(_actionModeWire, &QAction::triggered,
            [this] { _scene->setMode(QSchematic::Scene::WireMode); });
    ...
    connect(_actionRouteStraightAngles, &QAction::toggled,
            [this](bool checked) {
    ...
    connect(_actionGenerateNetlist, &QAction::triggered,

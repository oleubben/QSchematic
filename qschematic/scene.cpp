...
Scene::Scene(QObject *parent) : QGraphicsScene(parent) {
    ...
    m_wire_manager->set_net_factory(
        [this] { return std::make_shared<Items::WireNet>(); });
    connect(m_wire_manager.get(), &wire_system::manager::wire_point_moved, this,
            &Scene::wirePointMoved);

    // Undo stack
    _undoStack = new QUndoStack(this);
    connect(_undoStack, &QUndoStack::cleanChanged,
            [this](bool isClean) { Q_EMIT isDirtyChanged(!isClean); });
    ...
    connect(_popupTimer, &QTimer::timeout, [this] {
        ...
        // Get item popup
        _popup.reset(
            QGraphicsScene::addWidget(_highlightedItem->popup().release()));
...
void Scene::from_container(const gpds::container &container) {
    ...
    const std::size_t version =
        container.get_attribute<std::size_t>("version").value_or(-1);
    if (version != serdes_version)
        return;

    // Scene
    if (const gpds::container *sceneContainer =
            container.get_value<gpds::container *>("scene").value_or(nullptr);
        sceneContainer) {
        // Rect
        const gpds::container *rectContainer =
            sceneContainer->get_value<gpds::container *>("rect").value_or(
                nullptr);
        if (rectContainer) {
            QRect rect;
    ...
    // Items
    for (const auto &itemContainer :
         container.get_values<gpds::container *>("item")) {
        if (!itemContainer)
            continue;
    ...
    // Nets
    for (const gpds::container *netContainer :
         container.get_values<gpds::container *>("net")) {
        if (!netContainer)
            continue;
...
void Scene::setWireFactory(
    const std::function<std::shared_ptr<Items::Wire>()> &factory) {
...
int Scene::mode() const { return _mode; }

void Scene::toggleWirePosture() { _invertWirePosture = !_invertWirePosture; }
...
void Scene::clear() {
    // Ensure no lingering lifespans kept in map-keys, selections or
    // undocommands
    ...
    // Remove from scene
    // Do not use QGraphicsScene::clear() as that would also delete the items.
    // However, we still need them as we manage them via smart pointers (eg. in
    // commands)
    ...
    // Now that all the top-level items are safeguarded we can call the
    // underlying scene's clear()
...
bool Scene::addItem(const std::shared_ptr<Items::Item> &item) {
    // Sanity check
    if (!item)
        return false;
...
bool Scene::removeItem(const std::shared_ptr<Items::Item> item) {
    // Sanity check
    if (!item)
        return false;
    ...
    // NOTE: Sometimes ghosts remain (not drawn away) when they're active in
    // some way at remove time, found below from looking at Qt-source code...
    item->clearFocus();
...
QList<std::shared_ptr<Items::Item>> Scene::items() const { return _items; }

QList<std::shared_ptr<Items::Item>> Scene::itemsAt(const QPointF &scenePos,
                                                   Qt::SortOrder order) const {
    return ItemUtils::mapItemListToSharedPtrList<QList>(
        QGraphicsScene::items(scenePos, Qt::IntersectsItemShape, order));
}

QList<std::shared_ptr<Items::Item>> Scene::items(int itemType) const {
...
std::vector<std::shared_ptr<Items::Item>> Scene::selectedItems() const {
    return ItemUtils::mapItemListToSharedPtrList<std::vector>(
        QGraphicsScene::selectedItems());
}

/**
 * Returns only the selected items that are not part of another item.
 * \remark The top-level items are those that were added to the scene by calling
 * Scene::addItem. Items that should not be top-level items need to be added by
 * using QGraphicsScene::addItem
 * \returns List of selected top-level items
 */
std::vector<std::shared_ptr<Items::Item>> Scene::selectedTopLevelItems() const {
...
QList<std::shared_ptr<Items::Node>> Scene::nodes() const {
...
Scene::nodeFromConnector(const QSchematic::Items::Connector &connector) const {
    for (auto node : nodes()) {
        ...
        auto it = std::find_if(
            std::cbegin(connectors), std::cend(connectors),
            [&connector](const auto &c) { return c.get() == &connector; });
...
void Scene::undo() { _undoStack->undo(); }

void Scene::redo() { _undoStack->redo(); }

QUndoStack *Scene::undoStack() const { return _undoStack; }
...
void Scene::mousePressEvent(QGraphicsSceneMouseEvent *event) {
    ...
    case WireMode: {
        ...
        if (event->button() == Qt::LeftButton) {
            ...
            // Start a new wire if there isn't already one. Else continue the
            // current one.
            if (!_newWire) {
                _newWire = make_wire();
            ...
            for (const auto &node : nodes()) {
                for (const auto &connector : node->connectors()) {
                    ...
                    if (QVector2D(connector->scenePos() - snappedPos).length() <
                        1) {
                        m_wire_manager->attach_wire_to_connector(
                            _newWire.get(),
                            _newWire->pointsAbsolute().indexOf(snappedPos),
                            connector.get());
                        wireAttached = true;
                        break;
                    }
            ...
            for (const auto &wire : m_wire_manager->wires()) {
                ...
                if (wire->point_is_on_wire(_newWire->pointsAbsolute().last())) {
                    m_wire_manager->connect_wire(
                        wire.get(), _newWire.get(),
                        _newWire->pointsAbsolute().count() - 1);
                    wireAttached = true;
                    break;
                }
            }
...
void Scene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
    ...
    case NormalMode: {
        ...
        if (_movingNodes) {
            ...
            // Note: We want/need to ensure that all `Wire` items are first in
            // the list!
            ...
            std::partition(
                std::begin(itemsToMove), std::end(itemsToMove),
                [](const std::shared_ptr<Items::Item> &item) -> bool {
                    return std::dynamic_pointer_cast<Items::Wire>(item) !=
                           nullptr;
                });

            QVector2D moveBy;
            for (const auto &item : itemsToMove) {
                // Move the item if it is movable and it was previously
                // registered by the mousePressEvent
                moveBy =
                    QVector2D(item->pos() - _initialItemPositions.value(item));
            ...
            for (const auto &item : itemsToMove) {
                if (const Items::Node *node =
                        dynamic_cast<const Items::Node *>(item.get());
                    node)
...
void Scene::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
    ...
    case NormalMode: {
        ...
        // Note that we DO NOT want this in WireMode to prevent highlighting of
        // the wires during placing a new wire.
        ...
        if (event->buttons() & Qt::LeftButton) {
            ...
            if (_movingNodes) {
                // Get list of items that need to be moved
                // Note: We want/need to ensure that all `Wire` items are first
                // in the list!
                QVector<std::shared_ptr<Items::Item>> itemsToMove;
                for (const auto &item : selectedTopLevelItems()) {
                    // Ignore items not marked as movable
                ...
                for (const auto &item : itemsToMove) {
                    ...
                    QVector2D moveBy{_initialItemPositions.value(item) +
                                     newMousePos - _initialCursorPosition -
                                     item->pos()};
        ...
        Items::Item *item =
            dynamic_cast<Items::Item *>(itemAt(newMousePos, QTransform()));
        if (item && item->highlightEnabled()) {
            ...
            if (_highlightedItem) {
                ...
                Q_EMIT _highlightedItem->highlightChanged(*_highlightedItem,
                                                          false);
                _highlightedItem = {};
            }
    ...
    case WireMode: {
        ...
        // Transform mouse coordinates to grid positions (snapped to nearest
        // grid point)
        const QPointF &snappedPos = _settings.snapToGrid(event->scenePos());
        ...
        if (_settings.routeStraightAngles) {
            if (_newWireSegment) {
                ...
                point prevNode(_newWire->pointsAbsolute().at(
                    _newWire->pointsAbsolute().count() - 1));
                ...
                // Create the intermediate point that creates the straight angle
                point p1(_newWire->pointsAbsolute().at(
                    _newWire->pointsAbsolute().count() - 3));
                ...
                _newWire->move_point_to(_newWire->pointsAbsolute().count() - 2,
                                        p2);
                _newWire->move_point_to(_newWire->pointsAbsolute().count() - 1,
                                        p3);
            }
        } else {
...
void Scene::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event) {
    ...
    case WireMode: {
        ...
        if (_newWire && _newWire->pointsRelative().count() > 1) {
            ...
            // Get rid of the last point as mouseDoubleClickEvent() is following
            // mousePressEvent()
            _newWire->removeLastPoint();
...
void Scene::dropEvent(QGraphicsSceneDragDropEvent *event) {
    ...
    if (mimeData->hasFormat(Items::MIME_TYPE_NODE)) {
        ...
        const Items::MimeData *mimeData =
            qobject_cast<const Items::MimeData *>(event->mimeData());
        if (!mimeData)
            return;
...
QVector2D Scene::itemsMoveSnap(const std::shared_ptr<Items::Item> &items,
                               const QVector2D &moveBy) const {
    Q_UNUSED(items);

    return moveBy;
}

void Scene::updateNodeConnections(const Items::Node *node) {
    ...
    for (auto &connector : node->connectors()) {
        ...
        // Note: We only consider the first and the last point of a wire as new
        // legal connections
        for (const auto &wire : m_wire_manager->wires()) {
            int index = -1;

            if (wire->points().first().toPoint() ==
                connector->scenePos().toPoint())
                index = 0;
            else if (wire->points().last().toPoint() ==
                     connector->scenePos().toPoint())
            ...
            if (index != -1) {
                ...
                for (const auto &otherConnector : connectors()) {
                    ...
                    const auto crOther =
                        m_wire_manager->attached_wire(otherConnector.get());
                    if (!crOther)
                        continue;

                    if (cr) {
                        if (cr->wire == wire.get() &&
                            crOther->point_index == index) {
                ...
                if (!alreadyConnected) {
                    m_wire_manager->attach_wire_to_connector(wire.get(), index,
                                                             connector.get());
                    Q_EMIT netlistChanged();
                }
            }
    ...
    for (const auto &node : nodes()) {
        for (const auto &connector : node->connectors()) {
            ...
            if (cr->point_index == index) {
                if (connector->scenePos().toPoint() !=
                    rawWire.points().at(index).toPoint())
    ...
    for (const auto &node : nodes()) {
        for (const auto &connector : node->connectors()) {
            if (connector->scenePos().toPoint() == point.toPoint())
                m_wire_manager->attach_wire_to_connector(&rawWire, index,
                                                         connector.get());
        }
    }
...
void Scene::setupNewItem(Items::Item &item) {
    // Set settings
...
void Scene::generateConnections() {
    for (const auto &connector : connectors()) {
        std::shared_ptr<wire> wire =
            m_wire_manager->wire_with_extremity_at(connector->scenePos());
        if (wire)
            m_wire_manager->attach_wire_to_connector(wire.get(),
                                                     connector.get());
    }
...
std::shared_ptr<Items::Wire> Scene::make_wire() const {
    if (_wireFactory)
        return _wireFactory();
    else
...
QList<std::shared_ptr<Items::Connector>> Scene::connectors() const {
...
void Scene::itemHoverEnter(const std::shared_ptr<const Items::Item> &item) {
...
void Scene::itemHoverLeave(
    [[maybe_unused]] const std::shared_ptr<const Items::Item> &item) {
...
void Scene::removeUnconnectedWires() {
    ...
    for (const auto &wire : m_wire_manager->wires()) {
        ...
        for (const auto &connector : connectors()) {
            if (m_wire_manager->is_wire_attached_to(wire.get(),
                                                    connector.get())) {
                isConnected = true;
                break;
            }
...
bool Scene::addWire(const std::shared_ptr<Items::Wire> &wire) {
    if (!m_wire_manager->add_wire(wire))
        return false;

    // Add wire to scene
    // Wires created by mouse interactions are already added to the scene in the
    // Scene::mouseXxxEvent() calls. Prevent adding an already added item to the
    // scene
    if (wire->scene() != this) {
        if (!addItem(wire))
            return false;
...
void Scene::removeWire(const std::shared_ptr<Items::Wire> &wire) {

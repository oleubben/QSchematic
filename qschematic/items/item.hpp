...
class Item : public QGraphicsObject,
  ...
  public:
    ...
    explicit Item(int type, QGraphicsItem *parent = nullptr);
    ...
    template <typename RetT = Item> auto sharedPtr() -> std::shared_ptr<RetT> {
        if constexpr (std::is_same_v<RetT, Item>)
            return shared_from_this();
        else
    ...
    template <typename RetT = Item> auto weakPtr() -> std::weak_ptr<RetT> {
        if constexpr (std::is_same_v<RetT, Item>)
            return weak_from_this();
        else
    ...
    [[nodiscard]]
    virtual std::unique_ptr<QWidget> popup() const {
        return nullptr;
    }
